...
sw_int_t tcpss_res_work(sw_lsn_cycle_t *cycle, sw_loc_vars_t *locvar, sw_cmd_t *cmd)
{
	...
	ret = locvar->tostream(locvar, locbuf.data + 8, locbuf.size);
	if (ret < 0)
	{
	...
	locbuf.len = ret;
	locbuf.data[ret] = '\0';
	pub_log_bin(SW_LOG_INFO, locbuf.data, locbuf.len, "[%s][%d] Pack success! len=[%d]",
	...
	pub_log_debug("[%s][%d] Send message success! len=[%d]", __FILE__, __LINE__, locbuf.len);
...
sw_int_t tcpss_deal_req(sw_lsn_cycle_t *cycle, sw_buf_t *locbuf, int index)
{
	...
	ret = locvar.destream(&locvar, locbuf->data + 8);
	if (ret < 0)
	{
