...
sw_int_t mqla_init(sw_lsn_cycle_t *cycle)
{
	...
	else
	{
		...
		memset(conname, 0x00, sizeof(conname));
		sprintf(conname, "%s(%d)", cycle->lsn_conf.comm.group[0].remote.ip, cycle->lsn_conf.comm.group[0].remote.port);
		pub_log_debug("[%s][%d] connection name is [%s]", __FILE__, __LINE__, conname);

		ret = mq_connx(&g_qmgr.h_conn, g_qmgr.qmgr, conname, cycle->lsn_conf.comm.group[0].remote.service);
		if (ret < 0)
		{
...
static sw_int_t mqla_deal(sw_lsn_cycle_t *cycle)
{
	int	i = 0;
	int	ret = 0;
	int	timer = 0;
	...
	while (1)
	{
		...
		for (i = 0; i < recv_cnt; i++)
		{
			...
			if (ret != SW_OK)
			{
				pub_log_error("[%s][%d] event_handler error! ret=[%d]", __FILE__, __LINE__, ret);
			}
			pub_log_bend("\0");
		}
	}
