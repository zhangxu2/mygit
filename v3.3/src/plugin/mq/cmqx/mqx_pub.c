...
static	int 	g_mqencode = MQENC_NATIVE;

int mq_connx(MQHCONN *h_conn, char *qmgr_name, char *conn_name, char *channel_name)
{
	MQLONG	compcode = 0;
	MQLONG  reason = 0;
	MQCNO   conn_opt = {MQCNO_DEFAULT};
	MQCD	cli_conn = {MQCD_CLIENT_CONN_DEFAULT};
	
	if (qmgr_name == NULL || qmgr_name[0] == '\0')
	{
		pub_log_error("[%s][%d] qmgr_name can not be null!", __FILE__, __LINE__);
		return -1;
	}

	if (conn_name == NULL || conn_name[0] == '\0')
	{
		pub_log_error("[%s][%d] connection name can not be null!", __FILE__, __LINE__);
                return -1;
	}

	if (channel_name[0] == '\0')
	{
		strncpy(cli_conn.ChannelName, "SYSTEM.DEF.SVRCONN", MQ_CHANNEL_NAME_LENGTH);
		pub_log_debug("[%s][%d] using default channel:SYSTEM.DEF.SVRCONN", __FILE__, __LINE__);
	}
	else
	{
		strncpy(cli_conn.ChannelName, channel_name, MQ_CHANNEL_NAME_LENGTH);
	}

	strncpy(cli_conn.ConnectionName, conn_name, MQ_CONN_NAME_LENGTH);

	conn_opt.ClientConnPtr = &cli_conn;
	conn_opt.Version = MQCNO_VERSION_2;

	MQCONNX(qmgr_name, &conn_opt, h_conn, &compcode, &reason);
	if (compcode == MQCC_FAILED)
	{
		pub_log_error("[%s][%d] MQCONNX error! compcode=[%d] reason code=[%d]",
			__FILE__, __LINE__, compcode, reason);
		return -1;
	}

	if (getenv("MQENCODE") != NULL)
	{
		g_mqencode = atoi(getenv("MQENCODE"));
	}
	else
	{
		g_mqencode = MQENC_NATIVE;
	}
...
int mq_putmsg(MQHCONN h_conn, MQHOBJ h_obj, char *send_buf, int length, MQBYTE24 msgid, MQBYTE24 corrid, long exp_time)
{
	...
	pub_log_info("[%s][%d] compcode:reason=[%d]:[%d]",
                        __FILE__, __LINE__, compcode, reason);
	if (compcode != MQCC_OK)
	{
	...
	pub_log_debug("mq put message success");
	if (msgid != NULL)
	{
