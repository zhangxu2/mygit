...
sw_int_t jmsla_extcevt(sw_fd_list_t *fd_lists)
{
	return SW_DELETE;
}

sw_int_t jmsla_destroy()
{
	return 0;
}

sw_int_t jmsla_init(sw_lsn_cycle_t *cycle)
{
	int	ret = 0;
	
	memset(&g_jmsinfo, 0x0, sizeof(g_jmsinfo));
	char	*ptr = NULL;
	ptr = strchr(g_mqcfg.qmgr, '|');
	if (ptr == NULL)
	{
		pub_log_error("[%s][%d] QMGR [%s] format error, should jndifactory|url");
		return -1;
	}
	strncpy(g_jmsinfo.jndifactory, g_mqcfg.qmgr, ptr - g_mqcfg.qmgr);
	
	strcpy(g_jmsinfo.url, ptr + 1);
	pub_log_info("[%s][%d] jndifactory=[%s] url=[%s]", __FILE__, __LINE__, g_jmsinfo.jndifactory, g_jmsinfo.url);

	ptr = strchr(g_mqcfg.qm[0].qname, '|');
	if (ptr == NULL)
	{
		pub_log_error("[%s][%d] qname [%s] format error, should connfactory|jndiqueue!",
			__FILE__, __LINE__, g_mqcfg.qm[0].qname);
		return SW_ERROR;
	}
	strncpy(g_jmsinfo.connfactory, g_mqcfg.qm[0].qname, ptr - g_mqcfg.qm[0].qname);
	if (cycle->lsn_conf.sendtype == O_SEND)
	{
		strcpy(g_jmsinfo.jsendqueue, ptr + 1);
		pub_log_info("[%s][%d] JMS: url=[%s] connfacroty=[%s] jsendqueue=[%s]",
			__FILE__, __LINE__, g_jmsinfo.url, g_jmsinfo.connfactory, g_jmsinfo.jsendqueue);
	}
	else
	{
		strcpy(g_jmsinfo.jrecvqueue, ptr + 1);
		pub_log_info("[%s][%d] JMS: url=[%s] facroty=[%s] jrecvqueue=[%s]",
			__FILE__, __LINE__, g_jmsinfo.url, g_jmsinfo.connfactory, g_jmsinfo.jrecvqueue);
	}
	...
	if (cycle->lsn_conf.sendtype == O_SEND)
	{
		ret = jms_jmsinit(g_jmsinfo.jndifactory, g_jmsinfo.url, g_jmsinfo.connfactory,
			g_jmsinfo.jsendqueue, NULL, cycle->lsn_conf.sendtype);
	}
	else
	{	
		ret = jms_jmsinit(g_jmsinfo.jndifactory, g_jmsinfo.url, g_jmsinfo.connfactory,
			NULL, g_jmsinfo.jrecvqueue, cycle->lsn_conf.sendtype);
	}
	if (ret < 0)
	{
	...
	pub_log_info("[%s][%d] jmsla init jms success!", __FILE__, __LINE__);
	
	return SW_OK;
}

sw_int_t jmsla_deal_recv_timeout(sw_lsn_cycle_t *cycle)
{
	int	ret = 0;
...
sw_int_t jmsla_deal_send_timeout(sw_lsn_cycle_t *cycle)
{
	int	ret = 0;
...
sw_int_t jmsla_timeoutevt(sw_lsn_cycle_t *cycle)
{
	if (cycle == NULL)
	{
	...
	{
		return jmsla_deal_recv_timeout(cycle);
	}
	
	return jmsla_deal_send_timeout(cycle);
}

sw_int_t jmsla_search_index(sw_char_t *qname)
{
	int	i = 0;
	int	index = 0;
...
sw_int_t jmsla_call(sw_lsn_cycle_t *cycle, sw_loc_vars_t *vars, sw_buf_t *locbuf)
{
	int	ret = 0;
	int	index = 0;
	char	qname[64];
	...
	index = jmsla_search_index(qname);
	pub_log_info("[%s][%d] index=[%d]", __FILE__, __LINE__, index);
	...
	ret = jms_jmssend(locbuf->data);
	if (ret < 0)
	{
		pub_log_error("[%s][%d] JMS send message error!", __FILE__, __LINE__);
		return SW_ERROR;
	}
	pub_log_info("[%s][%d] jmsla_call success!", __FILE__, __LINE__);
	
	return SW_OK;
}

sw_int_t jmsla_deal_recv_work(sw_lsn_cycle_t *cycle, char *recvbuf, int recvlen, u_char *msgid, u_char *corrid)
{
	sw_int_t	ret = 0;
	sw_buf_t	locbuf;
	...
	locbuf.data = recvbuf;
	locbuf.len = recvlen;
	pub_log_bin(SW_LOG_DEBUG, locbuf.data, locbuf.len, "[%s][%d] Recv msg:[%d][%d]", 
		__FILE__, __LINE__, locbuf.len, strlen(recvbuf));
...
sw_int_t jmsla_recv_work(sw_lsn_cycle_t *cycle)
{
	int	i = 0;
	int	ret = 0;
	int	timer = 0;
	...
	while (1)
	{
		...
		for (i = 0; i < g_qmgr.qcnt; i++)
		{
			...
			ret = jms_jmsrecv(4000, recvbuf, &recvlen);
			if (ret < 0)
			{
			...
			ret = jmsla_deal_recv_work(cycle, recvbuf, recvlen, msgid, corrid);
			if (ret == -2)
			{
...
sw_int_t jmsla_send_work(sw_lsn_cycle_t *cycle)
{
	long		cmd_type = 0;
	sw_int_t	ret = 0;
	sw_int_t	len = 0;
	...
	switch (cmd.type)
	{
		...
		case SW_TASKDENY:
			...
			ret = jmsla_call(cycle, &locvar, &locbuf);
			if (ret != SW_OK)
			{
				err_flag = 1;
...
sw_int_t jmsla_interevt(sw_fd_list_t *fd_lists)
{
	...
	if (cycle->lsn_conf.sendtype == O_RECV)
	{
		return jmsla_recv_work(cycle);
	}
	
	return jmsla_send_work(cycle);
}
