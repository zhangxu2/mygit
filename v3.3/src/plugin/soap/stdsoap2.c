...
{ register struct soap_nlist *np = soap->nlist;
  ...
  { if (!(soap->mode & SOAP_XML_IGNORENS))
      if (np->index < 0
       || ((s = soap->local_namespaces[np->index].ns) && strcmp(s, np->ns)))
  ...
  if ((n1 == 3 && n1 == n2 && !strncmp(id1, "xml", 3) && !strncmp(id1, id2, 3))
   || (soap->mode & SOAP_XML_IGNORENS))
    return SOAP_OK;
	pub_log_error("[%s][%d] SOAP_SYNTAX_ERROR!", __FILE__, __LINE__);
...
#endif
      pub_log_debug("Accept socket %d at port %d from IP %s", soap->socket, soap->port, soap->host);
#ifndef WITH_LEAN
...
{ char header[SOAP_HDRLEN], *s;
  ...
  *soap->endpoint = '\0';
  ...
  { size_t l = 0;
	...
	pub_log_info("[%s][%d] endpoint=[%s] path=[%s] msgbuf=[%s]",
		__FILE__, __LINE__, soap->endpoint, soap->path, soap->msgbuf);
	...
	pub_log_info("[%s][%d] status=[%d]", __FILE__, __LINE__, status);
      if (status)
        return soap->error = status;
    }
    else if (status)
...
{ struct soap_multipart *content;
  ...
  { register char *key = soap->msgbuf;
    ...
    { *val = '\0';
      ...
      {
        content->disposition = soap_strdup(soap, val);
        content->filename = soap_strdup(soap, soap_get_header_attribute(soap, val, "name"));
      }
      else if (!soap_tag_cmp(key, "Content-Type"))
...
int
SOAP_FMAC2
soap_begin_serve_1(struct soap *soap)
{
#ifdef WITH_FASTCGI
  if (FCGI_Accept() < 0)
  { soap->error = SOAP_EOF;
...
{ soap->part = SOAP_IN_BODY;
  ...
  {
    pub_log_info("[%s][%d] begin in [SOAP->ENV:body] error!", __FILE__, __LINE__);
    return soap->error;
  }
  if (!soap->body)
...
{ if (soap_getheader(soap) && soap->error == SOAP_TAG_MISMATCH)
    soap->error = SOAP_OK;
   pub_log_info("[%s][%d] AAAAAAAAAAAA soap_recv_header soap->error=[%d] soap->tag=[%s]",
		__FILE__, __LINE__, soap->error, soap->tag);
...
/******************************************************************************/
int pub_dbgmsg(int flag, char *msg, int len)
{
	char buf[SW_LOG_MAX_MSG_STR];
	char *start = NULL, *end = NULL, *pos = NULL;
	if (len < SW_LOG_MAX_MSG_STR - 128)
	{
		pub_log_info(msg);
		return 0;
	}

	if (flag == SOAP_INDEX_SENT)
	{
		pub_log_info("[%s][%d] SOAP SEND LEN[%d]:", __FILE__, __LINE__, len);
	}
	else if (flag == SOAP_INDEX_RECV)
	{
		pub_log_info("[%s][%d] SOAP RECV LEN[%d]:", __FILE__, __LINE__, len);
	}
	start = pos = msg; 
	end = msg + len;
	while(pos < end)
	{
		pos += SW_LOG_MAX_MSG_STR - 128;
		if (pos > end)
		{
			pos = end;
		}

		memset(buf, 0x0, SW_LOG_MAX_MSG_STR);
