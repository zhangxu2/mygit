...
{ if (soap)\
  { if (!soap->fdebug[SOAP_INDEX_##DBGFILE])\
    ...
    { FILE *fdebug = soap->fdebug[SOAP_INDEX_##DBGFILE];\
      SOAP_MESSAGE(fdebug, "[%s][%d][%s] ", __FILE__, __LINE__, __FUNCTION__);\
      CMD;\
      fflush(fdebug);\
    }\
  }\
...
#else
# define DBGLOG(DBGFILE, CMD)
# define DBGMSG(DBGFILE, MSG, LEN)  pub_dbgmsg(SOAP_INDEX_##DBGFILE, MSG, LEN)
# define DBGFUN(FNAME)
...
{ struct soap_multipart *next;
  ...
  const char *disposition;		/* MIME Content-Disposition */
  const char *filename;			/* MIME Content-Disposition attribute name*/
#ifdef __cplusplus
...
SOAP_FMAC5 int SOAP_FMAC6 soap_serve(struct soap *soap, sw_loc_vars_t *vars, sw_xmltree_t *xml);
...
#endif
extern int pub_dbgmsg(int flag, char *msg, int len);
#endif /* STDSOAP_H */
