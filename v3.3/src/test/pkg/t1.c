...
int main(int argc, char* argv[])
{
	...
	loc_set_zd_data(&comm_vars, "#sys126", "126126126");
	...
	pkg_buf.len = pkg_integrate(&comm_vars, pkg_buf.data, config, 1);
	if (pkg_buf.len <= 0)
	{
	...
	result = pkg_analyze(&comm_vars, pkg_buf.data, config, pkg_buf.len, 1);
	if (result != SW_OK)
	{
