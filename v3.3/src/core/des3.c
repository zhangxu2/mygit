...
int des3_enc(unsigned char *input, unsigned char *output, int len)
{
	des3_context	ctx;
	unsigned char	des3key[32] = {"asdfghjklzxcvbnmqwertyui"};
	unsigned char	enc_iv[24] = {0, 00, 00, 00, 00, 00, 00, 00,};                                                     

	des3_set_3keys(&ctx,  (unsigned char *)des3key);
	des3_cbc_encrypt(&ctx, enc_iv, input, output, len);
	
	return 0;
}

int des3_dec(unsigned char *input, unsigned char *output, int len)
{
	des3_context	ctx;
	unsigned char	des3key[32] = {"asdfghjklzxcvbnmqwertyui"};
	unsigned char	dec_iv[24] = {0, 00, 00, 00, 00, 00, 00, 00,};                                                     

	des3_set_3keys(&ctx,  (unsigned char *)des3key);
	des3_cbc_decrypt( &ctx, dec_iv, input, output, len);
	
	return 0;
}

int pub_des3_enc(char *input, char *output, int len, char *key)
{
	des3_context	ctx;
	int		datalen = 0;
	char	data_default[1024 + 8];
	char	*data = NULL;
	char	des3key[32] = {"donghuaruanjianjinrongbu"};
	unsigned char	enc_iv[24] = {0, 00, 00, 00, 00, 00, 00, 00,};                                                     
	...
	memset(data, 0x0, len + 8);
	if (key != NULL && key[0] != '\0')
	{
		memset(des3key, 0x0, sizeof(des3key));
		strncpy(des3key, key, sizeof(des3key)-1);
	}

	des3_set_3keys(&ctx,  (unsigned char *)des3key);
	des3_cbc_encrypt(&ctx, enc_iv, (unsigned char *)input, (unsigned char *)data, len);
	datalen = (len % 8) ? (len / 8 + 1) * 8 : len;
	pub_code_bcdtoasc(data, output, datalen);
...
int pub_des3_dec(char *input, char *output, int len, char *key)
{
	des3_context	ctx;
	int		datalen = 0;
	char	data_default[1024 + 8];
	...
	memset(data, 0x0, datalen + 8);
	pub_code_asctobcd(input, data, len);
	...
	des3_cbc_decrypt( &ctx, dec_iv, (unsigned char *)data, (unsigned char *)output, datalen);
...
int pub_des3_enc_base64(char *input, char *output, int len, char *key, int keylen)
{
	des3_context	ctx;
	int		datalen = 0;
	char	data_default[1024 + 8];
	...
	if (input == NULL || output == NULL || len <= 0 || (keylen != 16 && keylen != 24))
	{
		pub_log_error("[%s][%d] param error", __FILE__, __LINE__);
		return -1;
	}
	...
	if (keylen == 16)
	{
		des3_set_2keys(&ctx,  (unsigned char *)des3key);
	}
	else if (keylen == 24)
	{
		des3_set_3keys(&ctx,  (unsigned char *)des3key);
	...
	pub_code_base64_enc(data, output, datalen);
...
int pub_des3_dec_base64(char *input, char *output, int len, char *key, int keylen)
{
	des3_context	ctx;
	int		datalen = 0;
	char	data_default[1024 + 8];
	...
	memset(data, 0x0, datalen + 8);
	if (pub_code_base64_dec(input, data, &datalen) < 0)
	{
		pub_log_error("[%s][%d] pub_code_base64_dec error", __FILE__, __LINE__);
		if (data != data_default)
		{
			free(data);
		}
