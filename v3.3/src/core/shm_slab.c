...
int shm_slab_init(shm_slab_pool_t *pool)
{
    ...
    pub_log_info("[%s][%d] page_num:%d!", __FILE__, __LINE__, page_num);
...
{
    int	pages = 0;
    u_char	*addr = NULL, *p = NULL;
    shm_slab_page_t *page = NULL;

    if(0 == size)
    {
        pub_log_error("[%s][%d] Size is incorrect!", __FILE__, __LINE__);
        return NULL;
    }
    ...
    if(size >= shm_slab_max_size())
    {
        ...
        if(NULL == page)
        {
            pub_log_error("[%s][%d] Alloc pages failed!", __FILE__, __LINE__);
            return NULL;
        }
...
static void *_shm_slab_alloc_slot(shm_slab_pool_t *pool, int slot_idx, int type)
{
    uintptr_t	*bitmap = NULL;
    shm_slab_slot_t *slot = NULL;
    shm_slab_page_t *start_page = NULL, *page = NULL, *new_page = NULL;
    u_char *addr = NULL, *data = NULL, *ptr = NULL;
    ...
    do
    {
        ...
        else if(exp_bitmaps > 0)
        {
            ...
            bitmap = (uintptr_t *)(data + (page->index << shm_slab_page_shift()));
    ...
    pub_log_error("[%s][%d] Alloc slot failed!", __FILE__, __LINE__);
    return NULL;
}
/******************************************************************************
    ...
    shm_slab_pool_t *pool, shm_slab_slot_t *slot, shm_slab_page_t *page)
{
    ...
    if(shm_slab_is_null_page(slot->page_idx))
    {
        pub_log_error("[%s][%d] Remove page failed!", __FILE__, __LINE__);
        return -1;
    }
    ...
    pub_log_error("[%s][%d] Search page failed!", __FILE__, __LINE__);
    return -1;
}
 ...
 **Author: # Qifeng.zou # 2013.07.12 #
 ******************************************************************************/
int shm_slab_free(shm_slab_pool_t *pool, void *vp)
{
    ...
    shm_slab_slot_t *slot = NULL;
    shm_slab_page_t *page = NULL;
    u_char *addr = NULL, *start = NULL, *p = vp;

    addr = (u_char *)pool;
    offset = (u_char *)p - addr;
    
    if((offset < pool->data_offset) || (offset > pool->end_offset))
    {
        pub_log_error("[%s][%d] Pointer address is incorrect!", __FILE__, __LINE__);
        return -1;
    }
    ...
    switch(page->type)
    {
        ...
        case SHM_SLAB_ALLOC_LARGE:
        {
            ...
            if(!(page->bitmap & page->rbitmap))
            {
                ...
                if(ret < 0)
                {
                    pub_log_error("[%s][%d] Remove page failed!",
                        __FILE__, __LINE__);
                    return -1;
                }
        ...
        default:
        {
            pub_log_error("[%s][%d] Type is unknown! [%d]",
                __FILE__, __LINE__, page->type);
            return -1;
        }
    }
